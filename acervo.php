...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($acao === 'buscar_aluno') {
        ...
        json_out(['success' => true, 'emprestimos' => $resultado]);
    }
...
// ════════════════════════════════════════════════════════════════════════════
// A partir daqui: apenas HTML (GET requests)
// ════════════════════════════════════════════════════════════════════════════
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  ...
  <style>
    ...
    .book-badges {
      display: flex; flex-wrap: wrap; gap: 0.3rem; margin: 0.45rem 0 0.35rem;
    }
    .book-badge {
      font-family: var(--font-mono); font-size: 0.8rem; letter-spacing: 0.1em;
      padding: 0.15rem 0.45rem; border-radius: 2px; text-transform: uppercase;
      white-space: nowrap;
    }
    ...
    <div class="modal-body">
      ...
      <div class="loans-header">
        ...
        <div class="loans-filters">
          ...
          <input class="loan-input filter-input" id="filter-data" type="text" placeholder="DD/MM/AAAA" maxlength="10" autocomplete="off">
        </div>
      </div>
...
(function () {
  ...
  async function buscarGrid(q) {
    try {
      ...
      else document.getElementById('grid').innerHTML = '<p class="empty-state">Erro ao carregar acervo.</p>';
    } catch (e) {
      console.error(e);
  ...
  function aplicarFiltros() {
    ...
    const dataBr = document.getElementById('filter-data').value;
    const dataISO = dataBr.length === 10 ? brToIso(dataBr) : '';
    document.querySelectorAll('#loans-list .loan-row').forEach(row => {
      ...
      const matchA   = !q || aluno.includes(q) || sala.includes(q);
      const matchD   = !dataISO || row.dataset.devol === dataISO;
      row.classList.toggle('hidden', !matchA || !matchD);
    });
  }
  ...
  async function abrirModal(registro) {
    ...
    document.getElementById('loan-devol').placeholder =
      `mín: ${pad(amanha.getDate())}/${pad(amanha.getMonth()+1)}/${amanha.getFullYear()}`;
